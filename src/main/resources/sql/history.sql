SELECT
  COALESCE(ln_victim.name, 'Unknown') AS victim_name,
...
FROM
  ...
  CROSS JOIN (SELECT COUNT(id) AS row_count FROM libertybans_simple_history WHERE type = ?) AS total_rows
WHERE
  lh.type = ?
ORDER BY
  lh.start DESC
